...
import '@holdfast-io/ui/styles.css'
import './index.css'

import { ErrorBoundary } from '@holdfast-io/react'
import {
  Badge,
  Box,
  Button,
              ...
              {editingConfig ? (
                ...
                <>
                  <Button
                    ...
                    emphasis={'medium'}
                    style={{ flexGrow: 1 }}
                    onClick={() => window.open(process.env.REACT_APP_FRONTEND_URI || 'http://localhost:3000', '_blank')}
                  >
                    Open HoldFast
                  </Button>
                  <ButtonIcon
...
const RecordingState = ({
  ...
  return (
    <Badge
      ...
      onClick={() =>
        session
          ? window.open(`${process.env.REACT_APP_FRONTEND_URI || 'http://localhost:3000'}/${projectId}/sessions/${session}`, '_blank')
          : null
      }
    />
  )
