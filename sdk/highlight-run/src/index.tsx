...
const H: HighlightPublicInterface = {
	...
	init: (projectID?: string | number, options?: HighlightOptions) => {
		try {
			...
			if (!projectID) {
				console.info(
					'HoldFast is not initializing because projectID was passed undefined.',
				)
				return
			}
	...
	start: (options) => {
		if (highlight_obj?.state === 'Recording' && !options?.forceNew) {
			if (!options?.silent) {
				console.warn(
					'HoldFast is already recording. Please `H.stop()` the current session before starting a new one.',
				)
			}
		} else {
			first_load_listeners.startListening()
	...
	stop: (options) => {
		if (highlight_obj?.state !== 'Recording') {
			if (!options?.silent) {
				console.warn(
					'HoldFast is already stopped. Please call `H.start()`.',
				)
			}
		} else {
