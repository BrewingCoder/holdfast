...
export const HighlightWarning = (context: string, msg: any) => {
	console.warn(`HoldFast Warning: (${context}): `, { output: msg })
}
...
export class Highlight {
	...
	_initMembers(options: HighlightClassOptions) {
		...
		this._backendUrl = options?.backendUrl ?? 'http://localhost:8082/public'
		...
		this.isRunningOnHighlight = false
	...
	identify(user_identifier: string, user_object = {}, source?: Source) {
		if (!user_identifier || user_identifier === '') {
			console.warn(
				`HoldFast's identify() call was passed an empty identifier.`,
		...
		try {
			...
			if (this._isCrossOriginIframe) {
				...
				if (
					...
					console.error(
						'Failed to initialize HoldFast; an error occurred on our end.',
						this.sessionData,
					)
					return
				}
...
SessionSecureID: ${this.sessionData.sessionSecureID}`,
			...
			if (this.options.disableSessionRecording) {
				this.logger.log(
					`HoldFast is NOT RECORDING a session replay per H.init setting.`,
				)
				this.ready = true
	...
	stopRecording(manual?: boolean) {
		...
		if (this.manualStopped) {
			this.addCustomEvent(
				'Stop',
				'H.stop() was called which stops HoldFast from recording.',
			)
		}
		this.state = 'NotRecording'
