...
export interface HighlightConfigOptions {
	...
	uploadSourceMaps?: boolean
	/**
	 * Configures a rewrite at /highlight-events for proxying HoldFast requests.
	 * @default true
	 */
	configureHighlightProxy?: boolean
	/**
	 * API key used to link to your HoldFast project when uploading source maps.
	...
	sourceMapsPath?: string
	/**
	 * Base path to append to your source map URLs when uploaded to HoldFast.
	 * @default '_next/'
	 */
	sourceMapsBasePath?: string
	/**
	 * Optional, backend url for private graph to use for uploading (for self-hosted HoldFast deployments).
	 */
	sourceMapsBackendUrl?: string
}
...
const getHighlightConfig = async (
	...
	if (defaultOpts.uploadSourceMaps || defaultOpts.configureHighlightProxy) {
		newRewrites = async () => {
			...
			const publicGraphUri = process.env.REACT_APP_PUBLIC_GRAPH_URI || 'http://localhost:8082/public'
			const otlpEndpoint = process.env.HIGHLIGHT_OTLP_ENDPOINT || 'http://localhost:4318'
			const highlightRewrites = [
				{
					source: '/highlight-events',
					destination: publicGraphUri,
				},
				{
					source: '/v1/traces',
