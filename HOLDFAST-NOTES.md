...
Default compose binds PostgreSQL on 5432 and Redis on 6379. If you have existing services on those ports, edit `docker/compose.yml` and remap:
...
The `docker/configure-collector.sh` script uses `[[ ]]` bash syntax but has a `#!/bin/sh` shebang. Alpine uses `ash`, which doesn't support `[[ ]]`. Fix by replacing all `[[ ]]` with POSIX `[ ]` syntax.

---

## What Was Stripped (from upstream Highlight.io)

All marketing, lead-gen, SaaS billing, and telemetry-home code has been removed. See `CHANGELOG-FORK.md` for full details.
...
All `AllWorkspaceSettings` boolean flags now default to `true` in `/backend/model/model.go`:
...
- **Backend AI code**: `/backend/private-graph/graph/resolver.go` (session insights), `/backend/private-graph/graph/schema.resolvers.go` (error suggestions)
- **Prompt templates**: `/backend/prompts/` directory
