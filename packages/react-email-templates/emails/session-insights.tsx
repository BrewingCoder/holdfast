...
import { EmailHtml, HighlightLogo, STATIC_ASSETS_BASE_URL } from '../components/common'
...
const sessionExample = {
	url: 'http://localhost:3000/1/sessions/123',
	identifier: 'jay@localhost',
	screenshotUrl: 'https://zane.test/404',
	activityGraphUrl:
		`${STATIC_ASSETS_BASE_URL}/assets/session-insights/activity.png`,
	avatarUrl:
		'https://lh3.googleusercontent.com/a-/AOh14Gg3zY3_wfixRrZjjMuj2eTrBAOKDZrDWeYlHsjL=s96-c',
	country: 'Germany',
	activeLength: '1h 25m',
	insights: [
		`The user viewed an error page with the URL 'http://localhost:3000/1/errors/7YboUB1obqGbOZV95X40gZrTGyRC/instances/196875448/?page=1&query=and%7C%7Cerror_state%2Cis%2COPEN'`,
...
export const SessionInsightsEmail = ({
	projectName = 'HoldFast Production',
	toEmail = 'admin@localhost',
	unsubscribeUrl = `${process.env.REACT_APP_FRONTEND_URI || 'http://localhost:3000'}/unsubscribe`,
	useHarold = false,
	...
	<EmailHtml previewText={`Session insights for ${projectName}`}>
		...
		<Text style={paragraph}>
			* These are sessions with unusual user journeys. You can read more
			...
			<Link
				style={anchor}
				href="http://localhost:3000/blog/interesting-sessions"
			>
				here
			</Link>
			.
		...
		{!useHarold && (
			<Text style={paragraph}>
				...
				<Link
					style={anchor}
					href={`${process.env.REACT_APP_FRONTEND_URI || 'http://localhost:3000'}/w/harold-ai`}
				>
					here
				</Link>
				.
		...
		<Img
			style={logoFull}
			src={`${STATIC_ASSETS_BASE_URL}/assets/digest/highlight-logo.png`}
			width="70"
			height="16"
			alt="HoldFast logo"
		/>
