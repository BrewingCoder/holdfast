...
export async function render(
	...
	const privateGraphUri = process.env.REACT_APP_PRIVATE_GRAPH_URI || 'http://localhost:8082/private'
	await page.goto(frontendUri)
	await page.setContent(getHtml(css, js))
	await setupOAuthProjectToken(page, {
		domain: apiConfig?.domain ?? new URL(privateGraphUri).host,
		oauthURL: apiConfig?.oauthURL ?? `${privateGraphUri}/oauth`,
		apiURL: apiConfig?.apiURL ?? privateGraphUri,
