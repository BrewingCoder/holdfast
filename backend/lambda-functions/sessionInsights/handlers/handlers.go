...
func formatSessionURL(projectId int, secureId string) string {
	...
	return fmt.Sprintf("%s/%d/sessions/%s", frontendUri, projectId, secureId)
}
...
func (h *handlers) SendSessionInsightsEmails(ctx context.Context, input utils.SessionInsightsData) error {
	...
	if input.DryRun {
		...
		toAddrs = []struct {
			AdminID int
			Email   string
		}{{AdminID: 5141, Email: "zane@holdfast.io"}}
	}
	...
	for _, toAddr := range toAddrs {
		...
		subject := fmt.Sprintf("[HoldFast] Session Insights - %s", input.ProjectName)
