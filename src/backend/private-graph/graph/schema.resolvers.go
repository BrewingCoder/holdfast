...
import (
	...
	"github.com/BrewingCoder/holdfast/src/backend/integrations/cloudflare"
	...
	"github.com/BrewingCoder/holdfast/src/backend/private-graph/graph/generated"
	modelInputs "github.com/BrewingCoder/holdfast/src/backend/private-graph/graph/model"
	"github.com/BrewingCoder/holdfast/src/backend/prompts"
...
func (r *mutationResolver) CreateWorkspace(ctx context.Context, name string) (*model.Workspace, error) {
	...
	// Self-hosted: no Stripe customer creation needed
...
func (r *mutationResolver) ExportSession(ctx context.Context, sessionSecureID string) (bool, error) {
	...
	tx := r.DB.WithContext(ctx).Model(&export).
...
func (r *mutationResolver) MarkErrorGroupAsViewed(ctx context.Context, errorSecureID string, viewed *bool) (*model.ErrorGroup, error) {
	...
	r.PrivateWorkerPool.SubmitRecover(func() {
		...
		if _, err := r.isUserInProject(ctx, eg.ProjectID); err != nil {
			log.WithContext(ctx).Infof("not adding error groups count to admin; this is probably a demo project, with id [%v]", eg.ProjectID)
			return
		}
	...
	`, eg.ID, admin.ID).Scan(&currentErrorGroupCount).Error; err != nil {
			...
			log.WithContext(ctx).Info("not updating error group count; admin has already viewed this error group")
			return
		}
...
func (r *mutationResolver) MarkSessionAsViewed(ctx context.Context, secureID string, viewed *bool) (*model.Session, error) {
	...
	r.PrivateWorkerPool.SubmitRecover(func() {
		...
		if _, err := r.isUserInProject(ctx, s.ProjectID); err != nil {
			log.WithContext(ctx).Infof("not adding session count to admin; this is probably a demo project, with id [%v]", s.ProjectID)
			return
		}
	...
	`, s.ID, admin.ID).Scan(&currentSessionCount).Error; err != nil {
			...
			log.WithContext(ctx).Info("not updating session count; admin has already viewed this session")
			return
		}
...
// Self-hosted: no Stripe subscriptions needed
...
// Self-hosted: AWS Marketplace is not used
...
// Self-hosted: no billing details to update
...
func (r *queryResolver) Accounts(ctx context.Context) ([]*modelInputs.Account, error) {
	...
	// Self-hosted: no Stripe subscription/invoice data; set defaults
	for _, account := range accounts {
		// Unlimited members and sessions for self-hosted
...
func (r *queryResolver) Events(ctx context.Context, sessionSecureID string) ([]any, error) {
	session, err := r.canAdminViewSession(ctx, sessionSecureID)
	if err != nil {
		return nil, err
	}
...
func (r *queryResolver) WebsocketEvents(ctx context.Context, sessionSecureID string) ([]any, error) {
	session, err := r.canAdminViewSession(ctx, sessionSecureID)
	if err != nil {
		return nil, err
	}
...
func (r *queryResolver) Resources(ctx context.Context, sessionSecureID string) ([]any, error) {
	s, err := r.canAdminViewSession(ctx, sessionSecureID)
	if err != nil {
		return nil, err
	}
...
func (r *queryResolver) CustomerPortalURL(ctx context.Context, workspaceID int) (string, error) {
	// Self-hosted: no Stripe customer portal
	return "", nil
}
	...
	`, admin.ID).Scan(&totalLogCount).Error; err != nil {
		...
		if err := r.DB.WithContext(ctx).Model(&model.LogAdminsView{}).Create(&model.LogAdminsView{
			...
			log.WithContext(ctx).
				...
				Error("error saving log view for admin")
		}
	})
...
func (r *queryResolver) KeyValuesSuggestions(ctx context.Context, productType modelInputs.ProductType, projectID int, dateRange modelInputs.DateRangeRequiredInput, keys []string) ([]*modelInputs.KeyValueSuggestion, error) {
	project, err := r.isUserInProjectOrDemoProject(ctx, projectID)
	if err != nil {
		return nil, err
	}
...
func (r *sessionResolver) UserObject(ctx context.Context, obj *model.Session) (any, error) {
...
func (r *sessionCommentResolver) Metadata(ctx context.Context, obj *model.SessionComment) (any, error) {
