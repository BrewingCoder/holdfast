...
func (store *Store) GetWorkspace(ctx context.Context, id int) (*model.Workspace, error) {
	return redis.CachedEval(ctx, store.Redis, fmt.Sprintf("workspace-id-%d", id), time.Second, time.Minute, func() (*model.Workspace, error) {
...
func (store *Store) GetWorkspaceAdminCount(ctx context.Context, id int) (int64, error) {
	value, err := redis.CachedEval(ctx, store.Redis, fmt.Sprintf("workspace-id-%d", id), time.Second, time.Minute, func() (*int64, error) {
