		...
		if len(turn.Activity.MembersRemoved) > 0 {
			for _, memberRemoved := range turn.Activity.MembersRemoved {
				if memberRemoved.ID == turn.Activity.Recipient.ID {
					...
					return turn.SendActivity(activity.MsgOptionText("HoldFast bot uninstalled successfully!"))
				}
			}
		}
		...
		if len(turn.Activity.MembersAdded) > 0 {
			...
			for _, member := range turn.Activity.MembersAdded {
				...
				if member.ID == turn.Activity.Recipient.ID {
					return turn.SendActivity(activity.MsgOptionText("👋 Your HoldFast notifications bot has been installed successfully. You can now set a Teams channel as recipient for your alerts."))
				}
			}
		}
