In HoldFast, you can search for a session by any of the data you send us (via the SDK)
...
By default, HoldFast will show completed sessions that have been fully processed, `completed=true`. For newer
projects with less sessions, HoldFast will show all sessions, and provide an example of how to use the search
...
For example, if you enter an expression without a key (`holdfast`) it will be used as the key for the following expression.

```
email=*holdfast* OR city=*holdfast*
```
...
```
identifier=spencer@holdfast.io
```

## Searching by User Clicks

When using HoldFast, you might be interested in querying for sessions where a user clicked a certain HTML element.
HoldFast records users' clicks on the page as two queryable properties: `clickSelector` and `clickInnerText`.
...
```
visited-url="https://app.holdfast.io/"
```
...
By default, HoldFast's SDKs will autoinject attributes to provide additional context to help with searching for sessions.
...
| `has_comments`      | If someone has commented on the HoldFast session   | `true`                                                                                                                                              |
