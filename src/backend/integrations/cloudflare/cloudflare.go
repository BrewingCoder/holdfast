...
async function forwardRequest(request, pathWithSearch) {
  ...
  return await fetch("%s", originRequest)
}
...
func GetScript() string {
	publicGraphUri := env.Config.PublicGraphUri
	if publicGraphUri == "" {
		publicGraphUri = "http://localhost:8082/public"
	}
	return fmt.Sprintf(ScriptTemplate, publicGraphUri)
}
