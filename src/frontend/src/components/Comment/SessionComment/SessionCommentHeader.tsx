...
const SessionCommentHeader: React.FC<Props> = ({ comment, isReply }) => {
	...
	const defaultIssueTitle = useMemo(() => {
		if (session?.identifier) {
			return `HoldFast Comment: ${session?.identifier}'s session`
		}
		if (session?.fingerprint) {
			return `HoldFast Comment: session with device ID ${session?.fingerprint}`
		}
		return `HoldFast Comment for a session`
	}, [session])
