...
COPY ./infra/docker/collector.yml /collector.yml
COPY ./infra/docker/configure-collector.sh /configure-collector.sh
...
COPY ./src/backend/localhostssl/server.crt /server.crt
COPY ./src/backend/localhostssl/server.key /server.key
